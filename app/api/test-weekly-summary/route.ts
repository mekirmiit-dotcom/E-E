...
async function sendPush(title: string, body: string, url: string) {
  ...
  const payload = JSON.stringify({ title, body, icon: "/icon-512.png", url })
...
export async function GET() {
  ...
  const { data: completedTasks } = await supabase
    .from("tasks")
    .select("id, owner, title, due_date, priority")
    .eq("status", "done")
    .gte("updated_at", weekStart.toISOString())

  const { data: overdueTasks } = await supabase
    .from("tasks")
    .select("id, title, owner, due_date, priority")
    .neq("status", "done")
  ...
  const { data: inserted, error } = await supabase
    .from("notifications")
    .insert({ task_id: null, message, type: "summary", read: false })
    .select("id")
    .single()

  if (error) {
    console.error("[test-weekly-summary] insert error:", error.message)
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
  }

  await sendPush(title, message, "/summary")
