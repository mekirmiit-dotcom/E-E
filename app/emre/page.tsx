...
export default function EmrePage() {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              >
                <div className="w-8 h-8 rounded-xl border border-slate-200 bg-white flex items-center justify-center group-hover:border-amber-300 transition-colors">
              ...
              <div>
                <h1 className="font-display font-bold text-base text-slate-900 leading-none">Emre</h1>
                <p className="text-[10px] font-mono text-muted-foreground">Kişisel Görev Panosu</p>
              </div>
            </div>
            <Button
              onClick={() => router.push("/tasks/new?owner=emre")}
              size="sm"
              className="gap-1.5 rounded-xl shadow-sm bg-amber-500 hover:bg-amber-600 text-white"
            >
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        {/* Profile Banner */}
        <div className="rounded-2xl bg-gradient-to-br from-amber-400 via-amber-500 to-amber-700 p-6 mb-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="font-display font-black text-2xl leading-none mb-1">Emre</h2>
              ...
              {totalTasks > 0 && (
                <div className="mt-3 w-full max-w-xs">
                  ...
                  <p className="text-[11px] text-amber-100 font-mono mt-1">{donePercent}% tamamlandı</p>
                </div>
              )}
            </div>
          </div>
            ...
            {filteredTasks.map((task) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left glass-card rounded-2xl p-4 hover:shadow-md transition-all hover:scale-[1.005] group border border-transparent hover:border-amber-100"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-medium text-sm text-slate-800 group-hover:text-amber-700 transition-colors leading-snug">
                        {task.title}
                      </h4>
                      {task.description && (
                      ...
                      {task.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {task.tags.slice(0, 3).map((tag) => (
                            <span key={tag} className="text-[10px] font-mono px-1.5 py-0.5 rounded-md bg-amber-50 text-amber-700 border border-amber-100">
                              #{tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                      ...
                      {task.notes && <span title="Not mevcut" className="text-amber-400"><StickyNote className="h-3.5 w-3.5" /></span>}
                  ...
                  {checkTotal > 0 && (
                    <div className="mt-3 pt-3 border-t border-slate-100">
                      ...
                      <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-amber-400 rounded-full transition-all duration-500" style={{ width: `${checkProgress}%` }} />
                      </div>
                    </div>
                  )}
                </button>
