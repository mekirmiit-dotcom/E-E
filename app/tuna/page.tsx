...
export default function TunaPage() {
  ...
  useEffect(() => {
    loadTasks().then((all) => {
      setTasks(all.filter((t) => t.owner === "tuna"))
      setMounted(true)
    })

    const channel = supabase
      .channel("tuna-tasks-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "tasks" },
        (payload) => { if ((payload.new as Task).owner === "tuna") setTasks((prev) => [...prev, payload.new as Task]) })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "tasks" },
        (payload) => setTasks((prev) => {
          const updated = payload.new as Task
          if (updated.owner !== "tuna") return prev.filter((t) => t.id !== updated.id)
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              >
                <div className="w-8 h-8 rounded-xl border border-slate-200 bg-white flex items-center justify-center group-hover:border-cyan-300 transition-colors">
                  <ArrowLeft className="h-4 w-4" />
                </div>
              </Link>
              <img src="/icon-512.png" alt="E&E" className="w-8 h-8 rounded-xl object-cover shadow-md" />
              <div>
                <h1 className="font-display font-bold text-base text-slate-900 dark:text-slate-100 leading-none">Tuna</h1>
                <p className="text-[10px] font-mono text-muted-foreground">Kişisel Görev Panosu</p>
              </div>
            </div>
            <ThemeToggle />
            <Button
              onClick={() => router.push("/tasks/new?owner=tuna")}
              size="sm"
              className="gap-1.5 rounded-xl shadow-sm bg-cyan-500 hover:bg-cyan-600 text-white"
            >
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        {/* Profile Banner */}
        <div className="rounded-2xl bg-gradient-to-br from-cyan-400 via-cyan-500 to-cyan-700 p-6 mb-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="font-display font-black text-2xl leading-none mb-1">Tuna</h2>
              <p className="text-cyan-100 text-sm font-body">
                {doneTasks}/{totalTasks} görev tamamlandı
                {overdueTasks > 0 && (
                  <span className="ml-2 text-red-200">• {overdueTasks} gecikmiş</span>
                )}
              </p>
              {totalTasks > 0 && (
                <div className="mt-3 w-full max-w-xs">
                  ...
                  <p className="text-[11px] text-cyan-100 font-mono mt-1">{donePercent}% tamamlandı</p>
                </div>
              )}
            </div>
          </div>
            ...
            {filteredTasks.map((task) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left glass-card rounded-2xl p-4 hover:shadow-md transition-all hover:scale-[1.005] group border border-transparent hover:border-cyan-100 dark:hover:border-cyan-900"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-medium text-sm text-slate-800 dark:text-slate-200 group-hover:text-cyan-700 dark:group-hover:text-cyan-300 transition-colors leading-snug">
                        {task.title}
                      </h4>
                      {task.description && (
                      ...
                      {task.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {task.tags.slice(0, 3).map((tag) => (
                            <span key={tag} className="text-[10px] font-mono px-1.5 py-0.5 rounded-md bg-cyan-50 text-cyan-700 border border-cyan-100">
                              #{tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                      ...
                      {task.notes && <span title="Not mevcut" className="text-cyan-400"><StickyNote className="h-3.5 w-3.5" /></span>}
                      <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-cyan-400 transition-colors" />
                    </div>
                  </div>
                  {checkTotal > 0 && (
                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                      ...
                      <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-cyan-400 rounded-full transition-all duration-500" style={{ width: `${checkProgress}%` }} />
                      </div>
                    </div>
                  )}
                </button>
