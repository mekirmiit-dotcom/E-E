...
import { Plus, LayoutGrid, Search, Filter, TrendingUp, Clock, AlertTriangle, CheckCircle2, RefreshCw } from "lucide-react"
...
import { loadTasks, updateTask, isOverdue, type Task } from "@/lib/tasks"
...
export default function DashboardPage() {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (overTask && overTask.owner === activeTask.owner && active.id !== overId) {
      ...
      for (const t of reordered) {
        await updateTask(t.id, { order_index: t.order_index })
      }
      return
    }

    setTasks(updatedTasks)
    await updateTask(active.id as string, { owner: overOwner })
  }
  ...
  const columns: {
    ...
    { owner: "emin", label: "Emin", emoji: "E", color: "bg-indigo-100 text-indigo-700", accentColor: "hover:bg-indigo-100 text-indigo-600" },
    { owner: "emre", label: "Emre", emoji: "E", color: "bg-amber-100 text-amber-700", accentColor: "hover:bg-amber-100 text-amber-600" },
    { owner: "shared", label: "Ortak", emoji: "🤝", color: "bg-emerald-100 text-emerald-700", accentColor: "hover:bg-emerald-100 text-emerald-600" },
  ]
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display font-bold text-base text-slate-900 leading-none">İş Takibi</h1>
            ...
            <div className="hidden md:flex relative w-64">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Görev ara..." className="pl-8 h-8 text-sm bg-slate-50/80 border-slate-200" />
            </div>

            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" onClick={fetchTasks} className="rounded-xl" title="Yenile">
                <RefreshCw className={cn("h-4 w-4 text-slate-500", loading && "animate-spin")} />
              </Button>
              <NotificationBell />
              <Button onClick={() => router.push("/tasks/new")} size="sm" className="gap-1.5 rounded-xl shadow-sm">
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
          ...
          {statusFilters.map((f) => (
            <button key={f.value} onClick={() => setFilterStatus(f.value)}
              className={cn("px-3 py-1.5 rounded-full text-xs font-medium font-body whitespace-nowrap transition-all",
                filterStatus === f.value ? "bg-indigo-600 text-white shadow-sm" : "bg-white/80 text-slate-600 hover:bg-white border border-slate-200"
              )}>
              {f.label}
            </button>
          ))}
          <div className="md:hidden flex-1 min-w-32 relative ml-2">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Ara..." className="pl-8 h-8 text-sm bg-white/80 border-slate-200" />
          </div>
        </div>
        ...
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {columns.map((col) => {
              ...
              return (
                <Column key={col.owner} owner={col.owner} tasks={colTasks} label={col.label}
                  emoji={col.emoji} color={col.color} accentColor={col.accentColor}
                  stats={{ total: allColTasks.length, done: allColTasks.filter((t) => t.status === "done").length, overdue: allColTasks.filter(isOverdue).length }}
                />
              )
            })}
          </div>
